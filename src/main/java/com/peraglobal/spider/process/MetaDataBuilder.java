...
public class MetaDataBuilder {
	 ...
	 * 功能:验证数据库链接
	 * <p>
	 * 作者 yongqian.liu 2016-12-15
	 */
	public static boolean isConn(DbConnection dbConnection) {
		try {
			Class.forName(dbConnection.getDriver());
			Connection conn = (Connection) DriverManager.getConnection(dbConnection.getUrl(), dbConnection.getUser(),
	...
	public static List getTables(DbConnection dbConnection) {
		try {
			String DBType = "mysql";
			Class.forName(dbConnection.getDriver());
			Connection con = (Connection) DriverManager.getConnection(dbConnection.getUrl(), dbConnection.getUser(),
					dbConnection.getPassword());
